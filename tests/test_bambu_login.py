...
def login_env(monkeypatch, tmp_path):
    ...
    # v2 keeps user state in BAMBU_STUDIO_AI_HOME; also hide any real config/legacy files.
    import common
    monkeypatch.setenv("BAMBU_STUDIO_AI_HOME", str(tmp_path))
    monkeypatch.setattr(common, "SKILL_DIR", str(tmp_path / "no-legacy-skill-dir"))
    monkeypatch.setattr(bambu, "_config", {})
    monkeypatch.setattr(bambu, "_secrets_loaded", True)
